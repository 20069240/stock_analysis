...
#Simple Moving Average
    df["SMA20d"] = np.round(df["Adj Close"].rolling(window = 20, center = False).mean(), 2)
    df["SMA50d"] = np.round(df["Adj Close"].rolling(window = 50, center = False).mean(), 2)
    df["SMA200d"] = np.round(df["Adj Close"].rolling(window = 200, center = False).mean(), 2)
    df['SMA20d-SMA50d'] = (df['SMA20d'] - df['SMA50d'])
#Exponential Moving Average
    df["EMA10d"] = pd.Series.ewm(df['Adj Close'], ignore_na=True,span=10,min_periods=9,adjust=True).mean()
    df["EMA50d"] = pd.Series.ewm(df['Adj Close'], ignore_na=False,span=50,min_periods=49,adjust=True).mean()
    df["EMA100d"] = pd.Series.ewm(df['Adj Close'], ignore_na=False,span=100,min_periods=99,adjust=True).mean()
#Bollinger Bands 
    df["STD20d"] = pd.Series.rolling(df['Adj Close'],window=20,center=False).std()
    df['Bollinger Band(+)'] = df["SMA20d"] + 2 * df["STD20d"]
    df['Bollinger Band(-)'] = df["SMA20d"] - 2 * df["STD20d"]
    df["B1"] = 4 * df["STD20d"] / df["SMA20d"]
    df["B2"] = (df['Adj Close'] - df["SMA20d"] + 2 * df["STD20d"]) / (4 * df["STD20d"])
    df["%B"] = (df['Adj Close'] - df['Bollinger Band(-)']) / (df['Bollinger Band(+)'] - df['Bollinger Band(-)'])
#Momentum
    df["MOM"] = df['Adj Close'].diff(10)  
#Rate of Change
    M = df['Adj Close'].diff(10 - 1)
    N = df['Adj Close'].shift(10 - 1)
    df["RoC"] = M / N
#Stochastic oscillator %K
    df["SO%k"] = (df['Adj Close'] - df['Low']) / (df['High'] - df['Low'])  
...
#MACD, MACD Signal and MACD difference
    EMAfast = pd.Series.ewm(df['Adj Close'], ignore_na=False,span=9,min_periods=25,adjust=True).mean()
    EMAsLow = pd.Series.ewm(df['Adj Close'], ignore_na=False,span=26,min_periods=25,adjust=True).mean()
...
#Force Index
    df["ForceIndex"] = pd.Series(df['Adj Close'].diff(10) * df['Volume'].diff(10), name = 'force' + str(10))
#Ease of Movement
...
#Commodity Channel Index
    df["PP"] = (df['High'] + df['Low'] + df['Adj Close']) / 3
...
#Coppock Curve
    M1 = df['Adj Close'].diff(int(20 * 11 / 10) - 1)
    N1 = df['Adj Close'].shift(int(20 * 11 / 10) - 1)
    ROC1 = M1 / N1
    M2 = df['Adj Close'].diff(int(20 * 14 / 10) - 1)
    N2 = df['Adj Close'].shift(int(20 * 14 / 10) - 1)
    ROC2 = M2 / N2
    df['CoppCurve'] = pd.Series.ewm(ROC1 + ROC2, ignore_na=False,span=20,min_periods=20,adjust=True).mean()
#Keltner Channel
    df['KelChM'] = pd.Series.rolling((df['High'] + df['Low'] + df['Adj Close']) / 3, window=10,center=False).mean()
    df['KelChU'] = pd.Series.rolling((4 * df['High'] - 2 * df['Low'] + df['Adj Close']) / 3, window=10,center=False).mean()
    df['KelChD'] = pd.Series.rolling((-2 * df['High'] + 4 * df['Low'] + df['Adj Close']) / 3, window=10,center=False).mean()      
#Chaikin Oscillator
    df['ADL'] = (2 * df['Adj Close'] - df['High'] - df['Low']) / (df['High'] - df['Low']) * df['Volume']
